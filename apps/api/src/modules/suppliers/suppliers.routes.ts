...
export default async function suppliersRoutes(fastify: FastifyInstance): Promise<void> {
  fastify.get('/api/suppliers', { preHandler: fastify.authenticate }, async (_request, reply) => {
    reply.status(200).send(await listSuppliers(fastify.prisma));
  });

  fastify.post('/api/suppliers', { preHandler: fastify.authenticate }, async (request, reply) => {
